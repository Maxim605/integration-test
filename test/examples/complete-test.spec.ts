...
describe("Complete test of all services", () => {
  ...
  describe("PostgreSQL, HTTP Mock, LDAP test", () => {
    ...
    it("должен иметь доступ к LDAP серверу", () => {
      const ldapInfo = manager.getServiceConnectionInfo("test-ldap-complete");
    ...
    it("должен иметь информацию о всех сервисах", () => {
      ...
      expect(servicesInfo["test-ldap-complete"]).toBeDefined();
      expect(servicesInfo["test-ldap-complete"].type).toBe("ldap");
    });
